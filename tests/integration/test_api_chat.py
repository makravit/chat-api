...
def test_chat_non_hello_message(client):
    token = get_token(client)
    resp = client.post("/api/v1/chat", json={"message": "How are you?"}, headers={"Authorization": f"Bearer {token}"})
    assert resp.status_code == 200
...
def test_chat_invalid_token(client):
    # Malformed token
    resp = client.post("/api/v1/chat", json={"message": "Hello!"}, headers={"Authorization": "Bearer invalid.token.value"})
    assert resp.status_code == 401
...
def test_chat_invalid_cases(client, payload, expected_error):
    token = get_token(client)
    resp = client.post("/api/v1/chat", json=payload, headers={"Authorization": f"Bearer {token}"})
...
def test_chat_service_exception(client, monkeypatch):
    ...
    with pytest.raises(Exception) as excinfo:
        client.post("/api/v1/chat", json={"message": "Hello!"}, headers={"Authorization": f"Bearer {token}"})
